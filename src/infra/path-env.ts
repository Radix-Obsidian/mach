...
function candidateBinDirs(opts: EnsureDotPathOpts): string[] {
  ...
  // Bundled macOS app: `dot` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "dot");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/dot` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "dot"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `dot` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureDotCliOnPath(opts: EnsureDotPathOpts = {}) {
  if (isTruthyEnvValue(process.env.DOT_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.DOT_PATH_BOOTSTRAPPED = "1";
