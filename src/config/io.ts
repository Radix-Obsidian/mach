...
import type { DotConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
function coerceConfig(value: unknown): DotConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as DotConfig;
}
...
function stampConfigVersion(cfg: DotConfig): DotConfig {
...
function warnIfConfigFromFuture(cfg: DotConfig, logger: Pick<typeof console, "warn">): void {
...
function applyConfigEnv(cfg: DotConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): DotConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as DotConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: DotConfig) {
    clearConfigCache();
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.DOT_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.DOT_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export async function writeConfigFile(cfg: DotConfig): Promise<void> {
  clearConfigCache();
