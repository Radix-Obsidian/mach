...
router.post("/missions", authenticateUser, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    console.log(`[Mach Missions] Created mission ${data.id} for user ${req.user.id}`);
...
// Get mission by ID (authenticated, validates ownership)
router.get("/missions/:id", authenticateUser, async (req: AuthenticatedRequest, res) => {
  const supabase = getSupabase();
  if (!supabase) {
    return res.status(503).json({ error: "Supabase not configured" });
  }
  ...
  if (req.user && data.owner_id && data.owner_id !== req.user.id) {
