...
async function setupMissionsTable() {
  ...
  // Test connection by checking if we can query the missions table
  const { data: existingMissions, error: queryError } = await supabase
    .from("missions")
    .select("id")
    .limit(1);

  if (queryError && queryError.code === '42P01') { // Table doesn't exist
    console.log("[DB Setup] Missions table doesn't exist yet. You'll need to create it manually.");
    console.log("[DB Setup] Please run this SQL in your Supabase SQL Editor:");
    console.log("");
    console.log(`
...
FOR SELECT USING (auth.role() = 'anon');
    `.trim());
    console.log("");
    console.log("[DB Setup] After creating the table, run this script again to add test data.");
    return;
  }

  if (queryError) {
    console.error("[DB Setup] Error checking table:", queryError);
    return;
  }

  console.log("[DB Setup] ✓ Database connection successful");
  console.log("[DB Setup] ✓ Missions table exists");
